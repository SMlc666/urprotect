...
- The independent musl-container profile uses the platform-specific ARM64 manifest digest `sha256:386b8fd9f78bb9aa7e5fb566656e0903518cb6f81d3b1520e1787a7dce1ef243` for `mcr.microsoft.com/dotnet/sdk:8.0-alpine` (resolved through the digest-only image reference). It bootstraps source with git inside the container, builds a musl PIE, runs the product CLI no-op path, and retains the digest/environment evidence.
