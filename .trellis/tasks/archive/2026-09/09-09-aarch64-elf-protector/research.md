...
- The independent musl-container profile uses the platform-specific ARM64 manifest digest `sha256:00c73d766c2c8fea23a9214954a5dec33c022c1a530c09e0f2a5e333fa4578ed` for the `mcr.microsoft.com/dotnet/sdk:8.0.424-bookworm-slim` image (resolved through the digest-only image reference). It installs pinned Debian musl tooling (`1.2.3-1`), bootstraps source with git inside the container, builds a musl PIE, runs the product CLI no-op path, and retains the digest/environment evidence.
- The Android container capability contract now pins the Waydroid ARM64 VANILLA system and MAINLINE vendor manifest hashes in `fixtures/manifest.json`; the hosted probe checks local archive paths when provisioned and remains informational while Binder/LXC/graphics or matching image files are unavailable.
