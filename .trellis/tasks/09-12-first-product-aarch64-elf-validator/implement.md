...
- [ ] Confirm the final CLI option grammar and documented exit-code matrix.
- [ ] Add product version constants and a single report schema-version owner.
- [ ] Add `ReportV1` DTOs and deterministic JSON serialization.
- [ ] Add human-output formatting that shares diagnostic projections with JSON.
...
- [ ] Replace ad hoc argument handling with a small testable command parser.
- [ ] Map core diagnostics and filesystem failures to exit codes 0/2/3/4/5/10.
- [ ] Add `--json <path|->` and keep stdout JSON-clean in stream mode.
- [ ] Atomically publish report files and preserve the existing no-op copy rules.
- [ ] Add tests for malformed options, missing values, missing input, output
...
- [ ] Project ELF identity, load-map/segment counts, dynamic metadata,
  symbols/version entries, notes/properties, relocation counts, analysis
  boundaries, and ordered diagnostics.
- [ ] Include source/output byte lengths and SHA-256 values when applicable.
- [ ] Add golden JSON files for synthetic valid, stripped, warning, and invalid
  cases.
- [ ] Verify repeated runs produce identical JSON apart from explicitly excluded
  path/timing fields.
